...
void RoutingProtocol::Start()
{
	...
	case ADD_EVC_USER:{
		...
		addnode->SetWaitStatus(true);
		break;
	}
	case FERRY_NODE:{
		// std::cout<<"ferry"<<std::endl;
		Ptr<MyUser> ferrynode = m_ipv4->GetObject<MyUser>();
		ferrynode->SetNodeType(FERRY_NODE);
		Ptr<Socket> socket;
		for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j) {
			socket = j->first;
		}
		ferrynode->SetSocket(socket);
		ferrynode->SetPatrolRoute(MyBuilding::GetPatrolRoute());
		
		// std::cout<<"saizu"<<ferrynode->GetPatrolRoute().size()<<std::endl;
		ferrynode->HandleFerryMovementTimer(Seconds(1.0));
		WriteAoiLog();
		WriteALLAoILog();
		WriteBlockAoiLog();
		if(m_flag){
			ActivateHelloTimer(MyBuilding::GetFerryHelloInterval());		
		}
		now = FERRY_NODE;
		break;
	}
...
void RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
	...
	}else if(nodeId >= MyBuilding::GetNumUsers()+MyBuilding::GetAddUsersNum() && nodeId < MyBuilding::GetNumUsers()+MyBuilding::GetAddUsersNum()+MyBuilding::GetExitNodes().size()){
		nodeType = EVC_AP;
	}else if(nodeId >= MyBuilding::GetNumUsers()+MyBuilding::GetAddUsersNum()+MyBuilding::GetExitNodes().size()){
		nodeType = FERRY_NODE;
	}
...
//	std::cout<<packet->RemoveHeader(tHeader)<<std::endl;
	...
	case TYPE_SHELTERDATA:
		...
		RecvShelterDataAsBroadcast(packet, receiver);
		break;
...
void RoutingProtocol::WriteBlockAoiLog(){
	...
	Ptr<MobilityModel> obj = user->GetObject<MobilityModel>();
	...
	std::string str = "obayashiIOFiles/Log/obayashi/" + mode + "/" + quickhello +"/" +eraseblock+ "/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/AoI_Info/Block_AoI_Info_"  +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	...
	for(auto a : blockdata){
		...
		fout << Simulator::Now().GetSeconds() <<","<< userID <<","<< x <<","<< y <<","<< Aoi <<","<< a.first <<std::endl;
...
void RoutingProtocol::WriteALLAoILog(){
	...
	std::map<String, float> blocked_data;
	blocked_data = user->GetBlockedNodeID_and_time_for_Log();
	...
	std::string str2 = "obayashiIOFiles/Log/obayashi/" + mode + "/" + quickhello +"/" +eraseblock+ "/"  + std::to_string(simple::MyBuilding::GetNumUsers()) + "/AoI_Info/ALL_AoI_Info_"  +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	...
	for(auto a : data){
		...
		fout2 << Simulator::Now().GetSeconds() <<","<< userID <<","<< x <<","<< y <<","<< Aoi <<","<< a.first <<std::endl;
	}
	// fout << std::endl;
	fout2.close();
	...
	std::string str3 = "obayashiIOFiles/Log/obayashi/" + mode + "/" + quickhello +"/" +eraseblock+ "/"  + std::to_string(simple::MyBuilding::GetNumUsers()) + "/AoI_Info/ALL_Blocked_AoI_Info_"  +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	str3.copy(filename3,str3.size());
	filename3[str3.size()] = '\0';
	std::ofstream fout3;
	fout3.open(filename3,std::ios::app);
	for(auto a : blocked_data){
		double Aoi = (Simulator::Now().GetDouble()/1000000000) - (a.second);

		// fout << "UserID:"<< userID <<", EXIT_ID:"<< a.first <<", Time:"<<a.second.second<<", AoI:"<< Aoi <<std::endl;
		fout3 << Simulator::Now().GetSeconds() <<","<< userID <<","<< x <<","<< y <<","<< Aoi <<","<< a.first <<std::endl;
	}
	// fout << std::endl;
	fout3.close();
...
void RoutingProtocol::WriteAoiLog(){
	...
	try{
		...
		// std::cout<<"rogudetenaiyozzz!!"<<std::endl;
		AoiTimer(1.0);
		return;
	}
	...
	fout << Simulator::Now().GetSeconds() << "," << userID << "," << x << "," << y <<"," << Aoi <<","<< exitnode <<std::endl;
	fout.close();
	////////////////////全避難所のAoIを取得///////////////////////
...
void RoutingProtocol::CheckMovingUserNum(){
	...
	std::string str = "obayashiIOFiles/Log/obayashi/" + mode + "/"  + quickhello +"/" +eraseblock+ "/"  + std::to_string(simple::MyBuilding::GetNumUsers()) + "/AoI_Info/moving_user_nun_"  +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	str.copy(filename,str.size());
	filename[str.size()] = '\0';
	std::ofstream fout;
	fout.open(filename,std::ios::app);
	...
	char filename2[1000];
	std::string str2 = "obayashiIOFiles/Log/obayashi/" + mode + "/" + quickhello +"/" +eraseblock+ "/" + std::to_string(simple::MyBuilding::GetNumUsers()) +  "/AoI_Info/all_moving_user_nun_"  +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	str2.copy(filename2,str2.size());
	filename2[str2.size()] = '\0';
	std::ofstream fout2;
	fout2.open(filename2,std::ios::app);
...
// 		std::string filename = "obayashiIOFiles/Log/obayashi/ON/testLog/testlog.txt";
...
void RoutingProtocol:: CheckHelloData(UserData data, Hop hop, Ipv4Address sender){
	bool sendflag = false;
	////////////////////////更新前の自分のデータ取得/////////////////////////////
	...
	std::map<uint32_t,std::pair<std::pair<String, uint32_t>,uint32_t>> m_hop = mydata->GetHop();
	...
	std::map<uint32_t, std::pair<std::pair<String, uint32_t>,uint32_t>> r_hop = hop.GetHop();
	...
	std::map<uint32_t, std::pair<std::pair<String, uint32_t>,uint32_t>> s_hop;
	...
	for(auto newexitdata : r_exit_aoi_data){
		...
		if(old_data_aoi > recv_data_aoi){ //データの生成時間で比較
			...
			for(auto a : r_hop){
				mydata->SetHop(a.first, a.second); //insertする
			}
			sendflag = true;
		...
		if(old_data_aoi < recv_data_aoi){
			std::pair<uint32_t, uint32_t> old_num_time = m_aoi_data.at(shelterID);
			...
			// mydata->SetHop(r_hop); //置き換える
			newdataflag = true;
			double increase_rate = double(old_num_time.first - recv_data_can_exitnum) / double(recv_data_aoi - old_num_time.second);
			// std::cout<<recv_data_aoi<<" - "<<old_num_time.second<<std::endl;
			// std::cout<<(old_num_time.first - recv_data_can_exitnum)<<"/"<<(recv_data_aoi - old_num_time.second)<<std::endl;
			mydata->SetIncreaseRate(shelterID, increase_rate);
		}
	}
	...
	for(auto b : r_aoi_blocked_data){
		...
		try{
			...
			if(eraseinfoflag == true){
				if(aoi < MyBuilding::GetEraseAoI()){ //しきい値より小さいならば新たな情報として設定する
					...
					mydata->SetBlockedNodeID_and_time_for_Log(blockID, recv_data_aoi);
		...
		// if((Simulator::Now().GetDouble()/1000000000) - (old_data_aoi) < 0){ //マイナスの値は未受信を示しているので、すぐに情報を更新する
		// 	//情報鮮度が高いデータ
		// 	double aoi = (Simulator::Now().GetDouble()/1000000000) - recv_data_aoi;
		// 	if(eraseinfoflag == true){
		...
		if(old_data_aoi < recv_data_aoi){
			...
			if(eraseinfoflag == true){
				if(aoi < MyBuilding::GetEraseAoI()){ //しきい値より小さいならばブロックとして設定する
					...
					m_aoi_blocked_data.insert(std::make_pair(blockID, recv_data_aoi));
					mydata->SetBlockedNodeID_and_time2(m_aoi_blocked_data); //更新されたデータをセット
					mydata->SetBlockedNodeID_and_time_for_Log2(m_aoi_blocked_data);
					mydata->UpdateGraphInfoReceivedPacket(blockID);//グラフを更新することで経路を更新？
				}else{
					// std::cout<<"Helloから通行止め箇所として設定しませんでした"<<std::endl; //debag
				}
...
//////////////////////////////////////////////////hopデータ比較////////////////////////////////////////////////////////////////
	...
	if(MyBuilding::GetSendHopFlag() == true){
		for(auto recvdata : r_hop){
			if(m_hop.find(recvdata.first) != m_hop.end()){
				std::pair<std::pair<String, uint32_t>, uint32_t> already_have_data = m_hop.at(recvdata.first); //所持しているHopデータに同じものが含まれるか確認
				if(recvdata.second.second > already_have_data.second){ 
					// std::cout<<"受信データの生成時間："<<recvdata.second.second<<",所持データの生成時間"<<already_have_data.second<<std::endl;
					// 受信データのほうが避難所までの所要時間が短い場合（受信データのほうが新しいデータの場合）
					// 上書き！！Update関数内でもデータが含まれるか確認しているので少し冗長ではある
					// std::cout<<"新しい情報を受信しました"<<std::endl;
					mydata->UpdateHop(recvdata.first, recvdata.second);
					newdataflag = true;
				}else if(recvdata.second.second == already_have_data.second){
					// std::cout<<"同じ生成時間のデータです"<<std::endl;
				}else{
					// 自分の所持しているデータのほうが受信したデータより避難所までの所要時間が短い場合（自分のほうが新しいデータを所持している場合）
					// std::cout<<"古いデータを受信しました"<<std::endl;
					s_hop.insert(std::make_pair(recvdata.first, already_have_data));
					sendflag = true;
				}
			}else{
				// std::cout<<"所持していないデータをHelloから受信！！"<<std::endl;
				mydata->SetHop(recvdata.first, recvdata.second);
				newdataflag = true;
			}
		}
		...
		for(auto d : m_hop){
			try{
				std::pair<std::pair<String, uint32_t>,uint32_t> compdata = r_hop.at(d.first); 
			}catch(std::out_of_range& oor){
				// 受信データに自分が所持しているデータが含まれていない場合（つまり送信もとが所持していない情報がある場合）
				// 送信もとに該当データのみを送る
				// 送り返すデータを作成
				s_hop.insert(d);
				sendflag = true;
		...
		mydata->SetEraseHopTimer();
	}
	// std::map<uint32_t, std::pair<std::pair<String, uint32_t>,uint32_t>> n_hop = mydata->GetHop(); //更新後の最新データを取得
	// for(auto a : n_hop){
	// 	if(a.second.second <= 0){
	// 		std::cout<<"到着予想が0秒後なので削除します"<<std::endl;
	// 		hopTimers[a.first].Cancel();
	// 		mydata->EraseHop(a.first); //到着予想時間が0のデータはすぐに削除する．
	// 	}else{
	// 		// std::cout<<a.second.second<<"秒後に情報を削除するタイマーをセットしました"<<std::endl;
	// 		EraseHopDataTimer(a.second.second, a.first);
	// 	}
	// }
	if(nodeType != FERRY_NODE){
		if(GetUseQuickHelloflag() == true && newdataflag == true && mydata->GetWhileQuickHelloFlag() == false){
			//新しい情報を受信してかつ，まだQuickHelloを実行していない場合はすぐにHelloを送る．
			mydata->SetWhileQuickHelloFlag(true);
			SendHello();
		}
	}
	...
	// 	sendflag = true;
	// }
	// if(mydata->GetHop().size() > 0){
	// 	std::cout<<"----------"<<mydata->GetUserId()<<"：HelloからのHopデータ-----------"<<std::endl;
	// 	for(auto a : mydata->GetHop()){
	...
	if(sendflag == true){
		...
		Hop hopdata(s_hop);
		SendUserDataAsUnicast(senddata, hopdata, dst);
	}

}

// void RoutingProtocol::CompareHopData(Hop r_hop, Hop m_hop){
...
UserData
RoutingProtocol::MakeSendDataFromRequest(Ipv4Address dst,
                                          ...
                                          std::vector<String> blockIds)
{
    ...
    std::set<String> sendBlockedSet;
    std::set<String> sendFullExitSet;
    std::map<String, std::pair<uint32_t, uint32_t>> sendExitInfo;
    std::map<String, float> sendBlockInfo;

    std::map<String, std::pair<uint32_t, uint32_t>> myExitInfo =
        user->GetAoiExitinfo();

    std::map<String, float> myBlockInfo =
        user->GetBlockedNodeID_and_time();

    for (auto id : exitIds) {
        if (myExitInfo.find(id) != myExitInfo.end()) {
            sendExitInfo.insert(std::make_pair(id, myExitInfo.at(id)));
        }
    }

    for (auto id : blockIds) {
        if (myBlockInfo.find(id) != myBlockInfo.end()) {
            sendBlockedSet.insert(id);
            sendBlockInfo.insert(std::make_pair(id, myBlockInfo.at(id)));
        }
    }
...
void RoutingProtocol::SendHello(){
	...
	for (auto a : aoi_exitinfo) {
		exitSummary.insert(std::make_pair(a.first, a.second.second));
	}

	std::map<String, float> blockSummary = aoi_blockinfo;

	exitSummary.clear();

	SummaryVector summary(userId, dst, exitSummary, blockSummary);
	summary.SetBroadcastId(m_broadcast_id);

	std::cout << "[SummaryVector] size = "
	...
	std::map <uint32_t, std::pair<std::pair<String, uint32_t>,uint32_t>> hop_map = user->GetHop();
	Hop hop(hop_map);		

	// if(nodeType == FERRY_NODE){
	// 	hello.SetMyPatrolErea_and_StartTime(user->GetMyPatrolErea_and_StartTime());
	// 	hello.SetOtherUAVPatrolErea_and_Id(user->GetOtherUAVPatrolErea_and_Id());
	// }

	hello.SetMyPatrolErea_and_StartTime(user->GetMyPatrolErea_and_StartTime());
	hello.SetOtherUAVPatrolErea_and_Id(user->GetOtherUAVPatrolErea_and_Id());
	hello.SetPatrolErea_and_Time(user->GetPatrolErea_and_Time());
	...
	for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
	{
		...
		if(packet->GetSize() > 65507){ //65507バイトまでしかHelloでは送れない
			std::cout<<"Helloで送信できるサイズを超えています！！！！！！！！！！！！！！！！！！！！！！"<<std::endl;
			std::cout<<"SendSize = "<<packet->GetSize()<<std::endl;
		}
		...
		// RoutingProtocol::OutputText_HelloSendTimeInfo(userId);
	...
	if(GetUseQuickHelloflag() == true && user->GetQuickSendflag() == true/*&& user->GetQuickHelloTimes() < MyBuilding::GetQuickHelloTimes()*/){
		...
		RoutingProtocol::OutputText_HelloSendTimeInfo(userId, true);
		return;
	}else{
		...
		//RoutingProtocol::OutputText_HelloSendTimeInfo(userId, false);
		//return;


    	double base = MyBuilding::GetHelloInterval(); 
    	double T = 50.0;                               
		uint32_t N = 0;

		Time now = Simulator::Now();

		for (std::map<uint32_t, Time>::iterator it = m_lastHelloSeen.begin();
     		it != m_lastHelloSeen.end();
     		++it)
		{
    		Time lastTime = it->second;

    		if (now - lastTime <= Seconds(T))
    		{
        		N++;  
    		}
		}
		...
		double interval = base * dense;

		if (interval > 10.0) interval = 10.0;

		ActivateHelloTimer(interval);

		user->SetAlreadyQuickSendFlag(false);
		user->InitQuickHelloTimes(); 
		RoutingProtocol::OutputText_HelloSendTimeInfo(userId, false);
		return;
	}
}
...
void
RoutingProtocol::SendRequest(Ipv4Address destination,
                             std::vector<String> exitIds,
                             std::vector<String> blockIds)
{
    RoutingTableEntry toDst;
    ...
    RequestData request(requesterId, exitIds, blockIds);

    Ptr<Packet> packet = Create<Packet>();
    packet->AddHeader(request);

    TypeHeader tHeader(TYPE_REQUEST);
    packet->AddHeader(tHeader);

	MyBuilding::PlusSendRequestNum();
	MyBuilding::PlusSendRequestBytes(packet->GetSize());

    std::cout << "[SendRequest] from userId = "
              << requesterId
              << ", to = "
              << destination
              << ", exit = "
    ...
    Simulator::Schedule(
        Time(MilliSeconds(m_uniformRandomVariable->GetInteger(0, 20))),
                             ...
                             Ipv4Address sender)
{
    ...
    RequestData request;
    p->RemoveHeader(request);

    MyBuilding::PlusRecvRequestNum();
    MyBuilding::PlusRecvRequestBytes(recvSize);

	if (IsNearShelter(200.0)) {
		MyBuilding::PlusNearShelterRecvRequestNum();
	}

    std::vector<String> exitIds = request.GetExitIds();
    std::vector<String> blockIds = request.GetBlockIds();

    std::cout << "[RecvRequest] from = "
              << sender
              << ", requesterId = "
    ...
    for (auto id : exitIds) {
        std::cout << "  requested exit: " << id << std::endl;
    }

    for (auto id : blockIds) {
        std::cout << "  requested block: " << id << std::endl;
    }

	BufferedRequest buffered;
	buffered.requesterAddress = sender;
	buffered.requesterId = request.GetRequesterId();
	buffered.exitIds = exitIds;
	buffered.blockIds = blockIds;

	m_requestBuffer.push_back(buffered);

	std::cout << "[BufferRequest] requesterId = "
			<< buffered.requesterId
			<< ", bufferSize = "
			<< m_requestBuffer.size()
			<< std::endl;

	if (!m_requestCollecting) {
		m_requestCollecting = true;

		std::cout << "[StartRequestCollecting] wait 1 sec" << std::endl;

		m_requestReplyEvent = Simulator::Schedule(
			Seconds(1.0),
			&RoutingProtocol::ProcessRequestBuffer,
			this
		);
	}
...
RoutingProtocol::ProcessRequestBuffer()
{
    ...
    BufferedRequest selected = m_requestBuffer.front();

    std::cout << "[ProcessRequestBuffer] buffered = "
              << m_requestBuffer.size()
              << std::endl;

    std::cout << "[SelectBufferedRequest] requesterId = "
              << selected.requesterId
              << ", address = "
              << selected.requesterAddress
              << ", exit = "
              << selected.exitIds.size()
              << ", block = "
              << selected.blockIds.size()
              << std::endl;

    for (auto id : selected.exitIds) {
        std::cout << "  selected exit: " << id << std::endl;
    }

    for (auto id : selected.blockIds) {
        std::cout << "  selected block: " << id << std::endl;
    }

	UserData senddata = MakeSendDataFromRequest(
		selected.requesterAddress,
		selected.exitIds,
		selected.blockIds
	);
	...
	std::cout << "[SendSelectedUserData] to requesterId = "
			<< selected.requesterId
			<< ", address = "
			<< selected.requesterAddress
			<< std::endl;
...
void merge_min(std::map<std::string, uint32_t>& a,
               const std::map<std::string, uint32_t>& b)
{
    for (const auto& kv : b) {
        const std::string& key = kv.first;
        uint32_t value_b = kv.second;
...
bool
RoutingProtocol::IsNearShelter(double threshold)
{
    Ptr<MyUser> user = m_ipv4->GetObject<MyUser>();
    ...
    std::map<String, double> distanceMap = user->GetExitDistanceMap();

    if (distanceMap.find(exitnode) == distanceMap.end()) {
        return false;
    }

    double distance = distanceMap.at(exitnode);
...
void RoutingProtocol::RecvHello(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender){
	// std::cout << "[RecvHello] Packet received from: " << sender << std::endl;
	// std::cout<<"PacketSize = "<<p->GetSize()<<", RecvHello!!!"<<std::endl;
	
	Ptr<MyUser> user = m_ipv4->GetObject<MyUser>();
	SummaryVector summary;
	Hop hop;
	p->RemoveHeader(summary);
	if(MyBuilding::GetSendHopFlag()){
		p->RemoveHeader(hop);
	}

	uint32_t id = summary.GetBroadcastId();

	//std::cout << "[Recv SummaryVector] sender = "
          //<< sender
          //<< ", userId = "
          //<< summary.GetUserId()
          //<< ", exitSummary = "
          //<< summary.GetExitSummary().size()
          //<< ", blockSummary = "
          //<< summary.GetBlockSummary().size()
          //<< std::endl;

	if (m_IdCache.IsDuplicate (sender, id))
	{
		std::cout<<"重複してるので削除します"<<std::endl;
		return;
	}

	RoutingProtocol::OutputText_HelloRecvTimeInfo(sender, receiver);
	MyBuilding::PlusRecvHelloNum();
	MyBuilding::PlusRecvHelloBytes(p->GetSize());

	if (IsNearShelter(200.0)) {
		MyBuilding::PlusNearShelterRecvHelloNum();
	}


	std::vector<String> requestExitIds;
	std::vector<String> requestBlockIds;

	std::map<String, uint32_t> r_exitSummary = summary.GetExitSummary();
	std::map<String, float> r_blockSummary = summary.GetBlockSummary();

	std::map<String, std::pair<uint32_t, uint32_t>> m_exitInfo = user->GetAoiExitinfo();
	std::map<String, float> m_blockInfo = user->GetBlockedNodeID_and_time();

	/*
	for (auto a : r_exitSummary) {
		String exitId = a.first;
		uint32_t recvTime = a.second;

		if (m_exitInfo.find(exitId) == m_exitInfo.end()) {
			requestExitIds.push_back(exitId);
		} else {
			uint32_t myTime = m_exitInfo.at(exitId).second;
			if (myTime < recvTime) {
				requestExitIds.push_back(exitId);
			}
		}
	...
	*/
	...
	for (auto a : r_blockSummary) {
		String blockId = a.first;
		float recvTime = a.second;

		if (m_blockInfo.find(blockId) == m_blockInfo.end()) {
			requestBlockIds.push_back(blockId);
		} else {
			float myTime = m_blockInfo.at(blockId);
			if (myTime < recvTime) {
				requestBlockIds.push_back(blockId);
			}
		}
	...
	//if (!requestExitIds.empty() || !requestBlockIds.empty()) {
	if (!requestBlockIds.empty()) {
		std::cout << "[Need Request] from sender = "
				<< sender
				<< ", exit = "
				<< requestExitIds.size()
				<< ", block = "
				<< requestBlockIds.size()
				<< std::endl;
		/*
		for (auto id : requestExitIds) {
			std::cout << "  need exit: " << id << std::endl;
		}
		*/
		for (auto id : requestBlockIds) {
			std::cout << "  need block: " << id << std::endl;
		}
		SendRequest(sender, requestExitIds, requestBlockIds);
	}

	uint32_t uavid = summary.GetUserId();
	//std::pair<String, uint32_t> r_erea = hello.GetMyPatrolErea_and_StartTime(); // 相手の担当エリアと開始時間
	//std::map<String, uint32_t> r_ereaid_time = hello.GetPatrolErea_and_Time(); // 相手が知っているエリアごとの開始時間
	//std::map<String, uint32_t> r_map = hello.GetOtherUAVPatrolErea_and_Id();   // 相手が知っている担当者マップ

	// 自分の情報を取得
	//std::map<String, uint32_t> m_map = user->GetOtherUAVPatrolErea_and_Id();
	//std::map<String, uint32_t> m_ereaid_time = user->GetPatrolErea_and_Time();

	// 1. 他のドローンの担当者マップをマージ
	//for(auto a : r_map){
		//m_map.insert(a); // insertはキーが存在する場合更新しないので注意が必要かも？
		// 確実に更新したい場合は m_map[a.first] = a.second; の方が安全な場合があります
	//}
	// 送信元(uavid)の現在の担当情報を確実に登録/更新
	//if (r_erea.first != "") { // 相手がどこかを担当している場合のみ
		//m_map[r_erea.first] = uavid; 
	//}

	// 2. エリアごとの開始時間をマージ (早い時間を優先 = merge_min)
	//merge_min(m_ereaid_time, r_ereaid_time);

	// 送信元の現在の担当情報を時間マップにも反映
	//if (r_erea.first != "") {
		// 既存の時間より早い、または未登録の場合に更新
		//if (m_ereaid_time.find(r_erea.first) == m_ereaid_time.end() || 
			//r_erea.second < m_ereaid_time[r_erea.first]) {
			//m_ereaid_time[r_erea.first] = r_erea.second;
		//}
	//}

	// 3. 更新したデータをUserにセット
	//user->SetOtherUAVPatrolErea_and_Id(m_map);
	//user->SetPatrolErea_and_Time(m_ereaid_time);


	// 4. 行動決定
	if(nodeType == FERRY_NODE){
		// ★ここで個別の判定は不要です★
		// データの更新が終わったので、ChangePatrolRouteを呼ぶだけで、
		// 内部の冒頭に追加した「継続判定ロジック」が
		// 「自分より早い時間が記録されている」ことを検知し、自動的に離脱処理を行います。
		
		user->ChangePatrolRoute(FROMRECV);
	}

	//miki
	m_lastHelloSeen[uavid] = Simulator::Now();

	// std::cout<<"recvhello"<<std::endl;
	//CheckHelloData(hello,hop,sender); //データと送信元のアドレスを返す
...
void RoutingProtocol::RecvShelterDataAsBroadcast(Ptr<Packet> p, Ipv4Address myAddress){
	...
	uint32_t broadcastid = data.GetBroadcastId();
	// user->ClearHop(); //hopのmapをクリア
	// Hopに自分の情報を追加（上書きされる）

	if (m_IdCache.IsDuplicate (data.GetId (), broadcastid))
	{
	...
	if(canexitnum_and_time.second < Time){
		...
		double increase_rate = double(canexitnum_and_time.first - can_exit_num) / double(Time - canexitnum_and_time.second); //1秒あたりの増加率
		user->SetIncreaseRate(s_shelterID, increase_rate);
		if(GetUseQuickHelloflag() == true && user->GetWhileQuickHelloFlag() == false && nodeType != FERRY_NODE){
			// CheckAoI();
			SendHello();
		}
	}else{
		std::cout<<"UserID = "<< user->GetUserId() + 1 <<", 避難所から古いデータを受信しました。(実行されるわけない)"<<std::endl;
	}
...
void RoutingProtocol::CheckAoI(){
	...
	for(auto a : AoIexitinfo){
		...
		if(AoI <= MyBuilding::GetAoIthreshold()){
			user->SetWhileQuickHelloFlag(true);
	...
	user->SetWhileQuickHelloFlag(false);
	user->SetQuickSendflag(false);

}

// void RoutingProtocol::EraseHopData(Ipv4Address key){
// 	Ptr<MyUser> user = m_ipv4->GetObject<MyUser>();
// 	std::map<Ipv4Address, std::pair<String, uint32_t>> m_hop;
// 	m_hop = user->GetHop();
// 	m_hop.erase(key);
// 	user->SetHop(m_hop);
// 	std::cout<<user->GetUserId()<<"：mapからkey:"<<key<<"の情報を削除しました"<<std::endl;
// }
...
void RoutingProtocol::ALLAoITimer(double delay)
{
	ALLAoiTimer.SetFunction(&RoutingProtocol::WriteALLAoILog, this);
	ALLAoiTimer.Remove();
	ALLAoiTimer.Schedule(Seconds(delay));
}
...
void RoutingProtocol::WriteLog ()
{
	...
	case ON:
		...
		break;
	case FERRY_NODE:
		OutputText_NI(Simulator::Now(), m_ipv4, "FERRY",m_parent,c_id,m_hop);
		break;
	case EXIT:
...
void RoutingProtocol::OutputText_HelloRecvTimeInfo(Ipv4Address sender, Ipv4Address reciver){
	char filename[1000];
	std::ostringstream oss_run;
	oss_run << RngSeedManager::GetRun();
	String mode="ON";
...
void RoutingProtocol::OutputText_HelloSendTimeInfo(uint32_t sender, bool quicksendflag){
	char filename[1000];
	std::ostringstream oss_run;
	oss_run << RngSeedManager::GetRun();
	String mode="ON";
...
//		std::string str = "ClusterViewer/"+oss_delay.str()+"/ClusterViewer_"+oss_run.str()+".txt";
	...
	std::ofstream fout;
	fout.open(filename,std::ios::app);
	fout << sender << ", "<< Simulator::Now().GetDouble()/1000000000 << ", " <<quicksendflag<<std::endl;
	fout.close();
}
...
void RoutingProtocol::WriteForFile_RW (std::string packetName, Ipv4Address dst, double time, uint64_t splitSecond)
{
	...
	std::string str = "obayashiIOFiles/Log/obayashi/" + mode + "/" + quickhello + "/" + eraseblock + "/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/ClusterViewer/ClusterViewer_" +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
//	std::string str = "obayashiIOFiles/Log/ClusterViewer/ClusterViewer_"+oss_run.str()+".txt";
	str.copy(filename,str.size());
	filename[str.size()] = '\0';
