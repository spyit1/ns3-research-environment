...
private:
	...
	static uint32_t send_request_num;
	static uint32_t send_request_bytes;
	static uint32_t recv_request_num;
	static uint32_t recv_request_bytes;

	static uint32_t send_userdata_num;
	static uint32_t send_userdata_bytes;
	static uint32_t recv_userdata_num;
	static uint32_t recv_userdata_bytes;

	static uint32_t near_shelter_recv_hello_num;
	static uint32_t near_shelter_recv_request_num;
	static uint32_t near_shelter_recv_userdata_num;
	...
	//2026-04-14
	static uint32_t addUsersPerInterval;   //
	static double addUserStartTime;        
	static double addUserEndTime;          
	static double measureStartTime;        
	static double measureEndTime;          
	static uint32_t nextAddUserIndex;

	// Active User
	static std::set<uint32_t> activeUsers;
...
public:
	static void AddActiveUser(uint32_t userId);
	static void RemoveActiveUser(uint32_t userId);
	static bool IsActiveUser(uint32_t userId);
	static uint32_t GetActiveUserNum();
	static std::set<uint32_t> GetActiveUsers();
	...
	static void WriteActiveUserLog();
	...
	static std::vector<uint32_t> GetRandomNum(){
		std::vector<uint32_t> vec;
		uint32_t addUsers = GetAddUsersNum();

		for (uint32_t i = 0; i < addUsers; i++) {
			vec.push_back(i);
		}
		return vec;
	...
	//static uint32_t GetUserDataSize(){ return UserDataSize; }

	static double GetAddUserInterval(){ return AddUserInterval; }
	...
	static void PlusSendRequestNum(){send_request_num++;}
	static void PlusRecvRequestNum(){recv_request_num++;}
	static void PlusSendRequestBytes(uint32_t bytes){send_request_bytes += bytes;}
	static void PlusRecvRequestBytes(uint32_t bytes){recv_request_bytes += bytes;}

	static void PlusSendUserDataNum(){send_userdata_num++;}
	static void PlusRecvUserDataNum(){recv_userdata_num++;}
	static void PlusSendUserDataBytes(uint32_t bytes){send_userdata_bytes += bytes;}
	static void PlusRecvUserDataBytes(uint32_t bytes){recv_userdata_bytes += bytes;}

	static void PlusNearShelterRecvHelloNum(){near_shelter_recv_hello_num++;}
	static void PlusNearShelterRecvRequestNum(){near_shelter_recv_request_num++;}
	static void PlusNearShelterRecvUserDataNum(){near_shelter_recv_userdata_num++;}

	static uint32_t GetSendRequestNum(){return send_request_num;}
	static uint32_t GetRecvRequestNum(){return recv_request_num;}

	static uint32_t GetSendUserDataNum(){return send_userdata_num;}
	static uint32_t GetRecvUserDataNum(){return recv_userdata_num;}	
	...
	static uint32_t GetNearShelterRecvHelloNum(){return near_shelter_recv_hello_num;}
	static uint32_t GetNearShelterRecvRequestNum(){return near_shelter_recv_request_num;}
	static uint32_t GetNearShelterRecvUserDataNum(){return near_shelter_recv_userdata_num;}
	...
	static uint32_t GetAddUsersPerInterval(){ return addUsersPerInterval; }
	static double GetAddUserStartTime(){ return addUserStartTime; }
	static double GetAddUserEndTime(){ return addUserEndTime; }
	static double GetMeasureStartTime(){ return measureStartTime; }
	static double GetMeasureEndTime(){ return measureEndTime; }

	static uint32_t GetNextAddUserIndex(){ return nextAddUserIndex; }
	static void SetNextAddUserIndex(uint32_t n){ nextAddUserIndex = n; }
	static void PlusNextAddUserIndex(){ nextAddUserIndex++; }
