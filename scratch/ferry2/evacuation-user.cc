...
void MyUser::CompleteEvacuation (String ID)
{
	...
	if (MyBuilding::IsActiveUser(m_userId)) {
            MyBuilding::RemoveActiveUser(m_userId);
        }
	...
	if (mob != 0) {
		...
		Waypoint outPoint(Seconds(now + 0.1), Vector(99999.0, 99999.0, 0.0));
		mob->AddWaypoint(outPoint);
	}
...
//2026-04-14
void MyUser::AddUserMovementTimer(Time delay)
{
	humTimer.SetFunction(&MyUser::HandleUserMovement, this);
	humTimer.Remove();
	humTimer.Schedule(delay);
	starttime = Simulator::Now().GetSeconds() + delay.GetSeconds();
}
