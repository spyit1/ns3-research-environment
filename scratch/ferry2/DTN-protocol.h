...
private:
	...
	bool m_requestCollecting = false;
	EventId m_requestReplyEvent;

	//miki 
	std::map<uint32_t, Time> m_lastHelloSeen;
...
private:
	...
	bool IsNearShelter(double threshold);
	...
	void SendRequest(Ipv4Address destination,
                 std::vector<String> exitIds,
                 std::vector<String> blockIds);
...
private:
	...
	UserData MakeSendDataFromRequest(Ipv4Address dst,
                                  std::vector<String> exitIds,
                                  std::vector<String> blockIds);
	...
	//void OutputText_SendUserDataInfo(uint32_t senderId, Ipv4Address dst, uint32_t size);
