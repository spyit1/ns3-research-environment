...
// std::cout<<"HandleUserMovement"<<std::endl;
	...
	exitnodeid=JSON_Data::GetIDfromCoordinate(wnlist.front().GetCoordinate());
	...
	if(MyBuilding::GetUseExitInfoFlag()){
		...
		for(auto exit : m_exitnodeinfolist){
			...
			if(target_node == zahyou){
				// std::cout<<"現在目指している避難所は"<<exit_node_id<<std::endl; //debag
				uint32_t remaining_capacity = m_aoi_exitinfo.at(exit_node_id).first; //現在の残りの収容人数を取得
				uint32_t evac_upper = MyBuilding::Get_first_exit_info().at(exit_node_id); //最初の収容人数制限
				uint32_t information_generation_time = m_aoi_exitinfo.at(exit_node_id).second; //情報の生成された時間(0秒が一番古い，数値が大きいほど新しい情報であることに注意)
				uint32_t aoi = (Simulator::Now().GetDouble()/1000000000) - information_generation_time;
				double rat = 0; //避難率
				if(remaining_capacity == 0){
					rat = 1;
				}else{
					// std::cout<<remaining_capacity<<", "<<evac_upper<<std::endl;
					rat = 1 - ((double)remaining_capacity/(double)evac_upper);
				}
				// std::cout<<"収容率："<<rat<<", 情報の生成時間："<<information_generation_time<<", AoI："<<aoi<<std::endl;

				double threshold = 1.0 - (0.95 - 0.1) * ((aoi/(Simulator::Now().GetDouble()/1000000000)) + (exit_distance/(MyBuilding::GetMaxDistanceToExit().at(exit_node_id))));
				// std::cout<<"しきい値："<<threshold<<std::endl;
				if(rat > threshold){
					// std::cout<<"収容率がしきい値を超えました"<<std::endl;
				}
...
void MyUser::CompleteEvacuation (String ID)
{
	...
	WriteLog::OutputText_CompleteTime(m_evacTime,JSON_Data::NodeIDstoi(m_exitnode),m_userId,m_changeroute ,m_changeexit);
