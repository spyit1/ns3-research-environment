    ...
    for(auto itr = no.begin();itr != no.end();++itr){
		...
		if(simple::MyBuilding::GetExitAutoflag()){
			...
			if(itr3!=enternode.end()){
				...
				if(cap_num == 4294967295) cap_num = 100; //元データが-1の場合避難可能人数を30に設定する
...
void NetSim::Initialize_flow_Moniter(){
	...
	oss_run << RngSeedManager::GetRun();
	// std::string str = "Log/Block_Nodes.txt";
	std::string str = "Log/obayashi/" + mode + "/" + quickhello + "/"+eraseblock+"/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/Info/Flow_moniter/flowMoniter_" +std::to_string(simple::MyBuilding::GetRun()) +".txt";
	str.copy(filename,str.size());
	filename[str.size()] = '\0';
	std::ofstream fout;
	fout.open(filename,std::ios::out);
...
void NetSim::Initialize_Logfile_for_AoiInfo(){
	...
	std::ofstream fout;
	fout.open(filename,std::ios::out);
	fout<<"userID,shelterID,time,AoI"<<std::endl;
	fout.close();


	char filename2[100];
	std::string str2 = "Log/obayashi/" + mode + "/" + quickhello + "/"+ eraseblock + "/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/AoI_Info/moving_user_nun_" +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	...
	char filename3[100];
	std::string str3 = "Log/obayashi/" + mode + "/" + quickhello + "/" + eraseblock + "/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/AoI_Info/all_moving_user_nun_" +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	...
	char filename4[100];
	std::string str4 = "Log/obayashi/" + mode + "/" + quickhello +"/" + eraseblock + "/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/AoI_Info/ALL_AoI_Info_"  +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	str4.copy(filename4,str4.size());
	filename4[str4.size()] = '\0';
	std::ofstream fout4;
	fout4.open(filename4,std::ios::out);
	fout4<<"userID,node,time,AoI"<<std::endl;
	fout4.close();

	char filename5[100];
	std::string str5 = "Log/obayashi/" + mode + "/" + quickhello +"/" + eraseblock + "/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/AoI_Info/Block_AoI_Info_"  +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	str5.copy(filename5,str5.size());
	filename5[str5.size()] = '\0';
	std::ofstream fout5;
	fout5.open(filename5,std::ios::out);
	fout5<<"userID,node,time,AoI"<<std::endl;
	fout5.close();
...
void NetSim::Initialize_Logfile_SpeedInfo(){
	...
	std::string str = "Log/obayashi/" + mode + "/" + quickhello +"/" +eraseblock+ "/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/Info/Speed/UserSpeed_"  +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	str.copy(filename,str.size());
	filename[str.size()] = '\0';
	std::ofstream fout;
	fout.open(filename,std::ios::out);
...
//	oss_delay << simple::MyBuilding::GetFloodingInterval();
	...
	char filename2[100];
	std::string str2 = "Log/obayashi/" + mode + "/" + quickhello + "/"+ eraseblock + "/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/testLog/testlog_" +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	// std::string str2 = "Log/obayashi/ON/testLog/testlog.txt";
	str2.copy(filename2,str2.size());
	filename2[str2.size()] = '\0';
	std::ofstream fout2;
	fout2.open(filename2,std::ios::out);
...
void NetSim::Initialize_Logfile_for_Block_Nodes(){
	...
	oss_run << RngSeedManager::GetRun();
	// std::string str = "Log/Block_Nodes.txt";
	std::string str = "Log/obayashi/" + mode + "/" + quickhello + "/" + eraseblock + "/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/Info/Block/BlockNode_" +std::to_string(simple::MyBuilding::GetRun()) +".txt";
	str.copy(filename,str.size());
	filename[str.size()] = '\0';
	std::ofstream fout;
	fout.open(filename,std::ios::out);
...
void NetSim::Initialize_Logfile_for_Exit_Nodes(){
	...
	std::ostringstream oss_run;
	oss_run << RngSeedManager::GetRun();
	// std::string str = "Log/Exit_Nodes.txt";
	std::string str = "Log/obayashi/" + mode + "/" + quickhello + "/" + eraseblock + "/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/Info/Exit/Exit_Nodes" +std::to_string(simple::MyBuilding::GetRun()) +".txt";
	str.copy(filename,str.size());
	filename[str.size()] = '\0';
	std::ofstream fout;
	fout.open(filename,std::ios::out);
...
void NetSim::OutputClusterMemberSize(){
	...
	std::string str = "Log/obayashi/" + mode + "/" + quickhello + "/" + eraseblock + "/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/ClusterInfo//ClusterMemberSize/ClusterMemberSize_" +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	str.copy(filename,str.size());
	filename[str.size()] = '\0';
...
void NetSim::OutputClusterHopInfo(){
	...
	std::string str = "Log/obayashi/" + mode + "/" + quickhello + "/" + eraseblock + "/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/ClusterInfo/ClusterHopInfo/ClusterHopInfo_" +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	str.copy(filename,str.size());
	filename[str.size()] = '\0';
...
void NetSim::OutputUserDataInfo(){
	...
	std::string str = "Log/obayashi/ON/"  +quickhello + "/" + eraseblock + "/" + std::to_string(simple::MyBuilding::GetNumUsers()) +"/Recive/All_User_Data_DTN_" +std::to_string(simple::MyBuilding::GetRun()) + ".txt";
	str.copy(filename,str.size());
	filename[str.size()] = '\0';
...
void NetSim::MakeNetworkTopologyforJSON()
{
	...
	//set APnodes
	// uint32_t APnum = simple::MyBuilding::GetExitNodes().size();
	// n_accessPoint = new Ptr<simple::MyAccessPoint> [APnum];
	...
	// add_user = new Ptr<simple::MyUser> [simple::MyBuilding::GetAddUsersNum()];
	// for (uint32_t i=0; i<simple::MyBuilding::GetAddUsersNum(); ++i)
	// {
	// 	n_user[i] = CreateObject<simple::MyUser> ();
	// 	adduserNodes.Add(add_user[i]);
	// }
	...
	//////////////////////////////////// random way select ///////////////////////////////////////////////
//	std::vector<int> rand = make_rand_array_select(simple::MyBuilding::GetNumUsers(),1,(int)simple::JSON_Data::get_linestrings().size());
...
//
////	n_user[0]->SetCurrentWay(way.at("way/98248559"));
////	n_user[0]->SetCurrentWay(way.at("way/28206994"));
////	n_user[0]->SetGraph(graph);
//
//	int i=0;
//	int j=0;
//
//	for(auto itr = way.begin();itr != way.end();++itr){
//		if(i==rand.front()){
//			n_user[j]->SetCurrentWay(way.at(itr->first));
...
//	for (uint32_t i=0; i<simple::MyBuilding::GetNumUsers(); ++i)
//	{
//		std::pair<double,double> n = n_user[i]->GetCurrentWay().GetNodes().front().GetCoordinate();
//		positionAlloc->Add (Vector (simple::JSON_Data::json_x(n.first), simple::JSON_Data::json_y(n.second), 0.0));
//	}
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	//////////////////////////////////// random node select ///////////////////////////////////////////////
//	std::vector<int> rand = make_rand_array_select(simple::MyBuilding::GetNumUsers(),0,(int)simple::JSON_Data::get_waynodes().size()-1);
 	// simple::MyBuilding::();
	// std::list<String> test_list = simple::MyBuilding::GetBlockedNodes();
...
//			std::cout<<"("<<coord.first<<","<<coord.second<<")"<<std::endl;
			...
			bool f_flag = false;
			bool b_flag = false;
			for(auto itr2 = w_nodes.begin();itr2 != std::prev(w_nodes.end());++itr2){
				simple::way_node n = *itr2;
				std::pair<double,double> c = n.GetCoordinate();
				if(c==coord){
					// auto itr3 = itr2; //自分の位置のイテレータを保持
...
//	// The propagation loss depends only on the distance between transfer and receiver
...
void OutputResult (uint32_t simtime)
{
	...
	//	// output txt file
		...
		std::string str_txt = "Log/obayashi/" + mode + "/" + quickhello + "/" + eraseblock + "/" + std::to_string(simple::MyBuilding::GetNumUsers()) + "/Result/SimulationResult_"+oss_run.str()+".txt";
	...
	//	char filename_pertimes_csv[100];
...
// end measuring simulation time
	...
	//////Lineへの終了通知//////////
	std::string command = "/bin/python3 /home/obayashi/ns-allinone-3.35/ns-3.35/LineNotify.py "+std::to_string(simple::MyBuilding::GetNumUsers())+' '+s;
	...
	String path = "Log/obayashi/ON/"+quickhello;
	String output_file = "zip/" + quickhello + "/" + usernum + "+" + addusernum + ".zip";
	command = "cd " + path + " && zip -rq ../../../../" + output_file + " " + usernum;
	// command = "zip -r zip/"+quickhello+"/"+usernum+"+"+addusernum+".zip Log/obayashi/ON/"+quickhello+"/"+usernum;
    system(command.c_str());
